 ...
 */
 ...
 import { ArrayUtils } from '@reparcar/common';
 ...
 const { workspaces = [] }: Pkg = require(path.resolve(rootPath, 'package.json'));
 ...
 const tsPkgs = workspaces
   ...
   .map((pt) => {
     ...
     const deps = ArrayUtils.unique(
       Object.keys(everyDeps).filter((dep) => everyDeps[dep].startsWith('workspace:'))
     );
 ...
 tsPkgs.forEach(({ name, dir, deps }) => {
   const isApp = !name.startsWith('@reparcar/');
   ...
   const paths = deps.reduce(
     (map: Record<string, string[]>, dep) => {
       const tsPkg = tsPkgs.find((tp) => tp.name === dep);

       if (tsPkg && tsPkg.name === '@reparcar/ui') {
     ...
     JSON.stringify(
       {
         ...
         include: ArrayUtils.unique([...includeExts, ...(existingTsConfig.include || [])]),
         references,
       },
       null,
       2
